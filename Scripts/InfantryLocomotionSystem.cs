...
public partial struct InfantryLocomotionSystem : ISystem
{
    [BurstCompile]
    public void OnUpdate(ref SystemState state)
    {
        new InfantryLocomotionJob
        {
    ...
    [BurstCompile]
    partial struct InfantryLocomotionJob : IJobEntity
    {
        public float DeltaTime;
        public float3 tempVelocity;
        public float speedMultiplier;

        public void Execute(ref LocalTransform transform, ref AgentBody body, in InfantryLocomotion locomotion, in AgentShape shape)
        {
            if (body.IsStopped)
                return;
